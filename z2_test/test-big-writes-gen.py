...
def get_numbers(bits, power, f_in):
    while True:
...
with open(sys.argv[1], 'wb') as f_in:
    out1 = get_numbers(BITS*2, 32, f_in)
    out2 = get_numbers(BITS*4, 16, f_in)
    out3 = get_numbers(BITS*8, 8, f_in)
    out4 = get_numbers(BITS*1, 64, f_in)
